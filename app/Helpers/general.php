...
if ( ! function_exists('get_apps')){
    function get_apps($value = '', $field = 'id'){
        if($value == ''){
            $apps = \App\Models\AppModel::where('status', 1)->get();
            return $apps;
        }else{
            $app = \App\Models\AppModel::where('status', 1)->where($field, $value)->firstOrFail();
            return $app;
        }
    }
}
...
if ( ! function_exists('cache_clear')){
    function cache_clear(){
        ...
        $apps = App\Models\AppModel::orderBy('id', 'DESC')->get();
        ...
        foreach ($apps as $key => $app) {
            ...
            $live_matches = App\Models\LiveMatch::select('live_matches.*')
...
if ( ! function_exists('getGeneratedToken')){
    function getGeneratedToken($source_url, $stream, $userIP)
    {
        


        $flussonic = $source_url;
        $key = ENV('FIL_KEY');
        $lifetime = 3600 * 3;
